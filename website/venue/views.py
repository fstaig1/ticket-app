...
def venue():
    ...
    return render_template('venue.html', user=user, venue=venue, concerts=concerts, form=form)
...
def create_concert():
    ...
    artist = Artist.query.filter_by(name=form.artistName.data).first()
    if not artist:
    ...
    venue.create_Concert(artistId=artist.id,
                         ...
                         date=datetime.strptime(form.date.raw_data[0], '%Y-%m-%dT%H:%M'))
